...
for i, p in enumerate(vor.points):
  if (0 <= p[0] <= 1) and (0 <= p[1] <= 1) and -1 not in vor.regions[vor.point_region[i]]:
    ...
    c = math.sqrt((x - 128)**2 + (y - 128)**2)
    #land[i] = c < 80 + np.random.randint(-70, 70)
    a = int(math.atan2(y - 128, x - 128) / (math.pi / 6))
    land[i] = c < R[a]
...
def onpick(event):
  ...
  land[ind] = 1-land[ind]
  if land[ind] == 1:
    polygon = [vor.vertices[v] for v in vor.regions[vor.point_region[ind]]]
...
#animation = FuncAnimation(fig, fill_polygons, interval=10)
plt.show()
